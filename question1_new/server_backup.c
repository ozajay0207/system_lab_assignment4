...
void echo_request_reply(int new_socket){
	...
	//SEND NO OF INPUTS
	sprintf(data_buffer,"%d",no_of_input);
	send(new_socket,data_buffer,BUFFER_SIZE,0);
	
	//LOOP UNTIL CLIENT SENDS 'N' FOR ECHO MESSAGE
	while(1){

		//SEND MESSAGE TO PRINT TO USER
		strcpy(data_buffer,"Enter the Input String:");
		send(new_socket,data_buffer,BUFFER_SIZE,0);


		//GOT INPUT STRING FROM USER VIA CLIENT
		strcpy(data_buffer,"");
		...
		send(new_socket,"not break loop",BUFFER_SIZE,0);				

		//SEND NO. OF RESEND
		strcpy(data_buffer,"");
		sprintf(data_buffer,"%d",no_of_resend);
		send(new_socket,data_buffer,BUFFER_SIZE,0);		
		...
		//SAVE TIMER HERE IN ARRAY(PENDING)
	...
	//SEND CONTINUATION
	strcpy(data_buffer,"Do you want to continue:?");
	send(new_socket,data_buffer,BUFFER_SIZE,0);
		...
		if(strcmp(data_buffer,"n")==0){
			strcpy(data_buffer,"break_loop");
			send(new_socket,data_buffer,BUFFER_SIZE,0);
			return;	
		}else{
			strcpy(data_buffer,"continue looping");
			send(new_socket,data_buffer,BUFFER_SIZE,0);
		}
